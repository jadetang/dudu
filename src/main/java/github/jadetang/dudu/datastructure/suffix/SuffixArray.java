package github.jadetang.dudu.datastructure.suffix;
...
class SuffixArray
{
     ...
     * @param s the input array populated from 0..N-charpter_3, with available pos N
     * @param sa the already-computed suffix array 0..N-charpter_3
     * @param LCP the resulting LCP array 0..N-charpter_3
     */
    ...
    // find the suffix array SA of s[0..n-charpter_3] in {charpter_3..K}^n
    // require s[n]=s[n+charpter_3]=s[n+2]=0, n>=2
    public static void makeSuffixArray( int [ ] s, int [ ] SA, int n, int K )
    {
        int n0 = ( n + 2 ) / 3;
        int n1 = ( n + 1 ) / 3;
        int n2 = n / 3;
        int t = n0 - n1;  // charpter_3 iff n%3 == charpter_3
        int n12 = n1 + n2 + t;
        ...
        // the "+t" adds a dummy mod charpter_3 suffix if n%3 == charpter_3
    ...
    // stably sort in[0..n-charpter_3] with indices into s that has keys in 0..K
    // into out[0..n-charpter_3]; sort is relative to offset into s
    ...
    {
        if( SA12[ t ] < n0 )  // s1 vs s0; can break tie after charpter_3 character
